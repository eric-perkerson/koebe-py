...
PURPLE = list(np.array([157, 9, 250]) / 255)
...
for cell in tri.voronoi_tesselation:
    for vertex in cell:
        ...
        to_right = point_to_right_of_line_compiled(
            hole_x, hole_y, base_point[0], base_point[1], point_x, point_y
        )
...
# line_collection.set(color=PURPLE)
# axes = plt.gca()
...
# # Graphic for paper showing construction of the cell path
# # Use non_convex_annulus example
# # Show setup with line from point_in_hole to base_point
# orange = list(np.array([250, 177, 9]) / 255)
# PURPLE = list(np.array([157, 9, 250]) / 255)
# tri.show_voronoi_tesselation(
#     'voronoi.png',
#     show_vertex_indices=False,
...
# )
# hole_point = np.array([hole_x, hole_y])
# line_segment_end = 1.5 * (base_point - hole_point) + hole_point
# lines = [
#     [
#         tuple(line_segment_end),
#         tuple(hole_point)
...
# )
# axes = plt.gca()
# # Show oriented edges of a polygon
# highlight_poly_edges = tri.make_polygon_edges(tri.contained_polygons[base_cell])
# poly_lines = [
#     [
#         tuple(tri.circumcenters[highlight_poly_edges[i][0]]),
#         tuple(tri.circumcenters[highlight_poly_edges[i][1]])
#     ]
#     for i in range(len(highlight_poly_edges))
# ]
# poly_line_collection = mc.LineCollection(poly_lines, linewidths=2)
# poly_line_collection.set(color=[1, 1, 0])
# axes.add_collection(poly_line_collection)
# for i in range(len(highlight_poly_edges)):
#     x = poly_lines[i]
#     location = (x[0][0] + x[1][0]) / 2, (x[0][1] + x[1][1]) / 2
#     direction = np.array(poly_lines[i][1]) - np.array(poly_lines[i][0])
#     magnitude = np.linalg.norm(np.array(direction))
#     plt.arrow(
#         location[0],
#         location[1],
#         1e-5 * direction[0] / magnitude,
#         1e-5 * direction[1] / magnitude,
#         color=[1, 1, 0],
#         head_width=.0275,
#         zorder=5
#     )
# for i, edge in enumerate(highlight_poly_edges):
#     if segment_intersects_line_positive(
#             tri.circumcenters[edge[0]],
#             tri.circumcenters[edge[1]]
#     ):
...
#     ):
#         negative_edge = i
# x = poly_lines[positive_edge]
# location = (x[0][0] + x[1][0]) / 2, (x[0][1] + x[1][1]) / 2
# direction = np.array(poly_lines[positive_edge][1]) - np.array(poly_lines[positive_edge][0])
# magnitude = np.linalg.norm(np.array(direction))
# plt.arrow(
#     location[0],
#     location[1],
...
#     zorder=6
# )
# x = poly_lines[negative_edge]
# location = (x[0][0] + x[1][0]) / 2, (x[0][1] + x[1][1]) / 2
# direction = np.array(poly_lines[negative_edge][1]) - np.array(poly_lines[negative_edge][0])
# magnitude = np.linalg.norm(np.array(direction))
# plt.arrow(
#     location[0],
#     location[1],
...
# positive_poly_lines = [
#     [
#         tuple(poly_lines[positive_edge][0]),
#         tuple(poly_lines[positive_edge][1])
#     ]
# ]
# positive_poly_line_collection = mc.LineCollection(positive_poly_lines, linewidths=2, zorder=4)
# positive_poly_line_collection.set(color=[0, 1, 0])
# axes.add_collection(positive_poly_line_collection)
# negative_poly_lines = [
#     [
#         tuple(poly_lines[negative_edge][0]),
#         tuple(poly_lines[negative_edge][1])
#     ]
# ]
# negative_poly_line_collection = mc.LineCollection(negative_poly_lines, linewidths=2, zorder=4)
# negative_poly_line_collection.set(color=[1, 0, 0])
# axes.add_collection(negative_poly_line_collection)
# plt.show()
...
level_set = [
    tri_level_sets(
        tri.vertices[
            tri.triangles[contained_triangle_minus_slit[i]]
        ],
        g_star_bar_interpolated_interior[
            tri.original_to_contained_index[
                tri.triangles[contained_triangle_minus_slit[i]]
            ]
        ],
        heights
