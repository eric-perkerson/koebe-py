...
class Triangulation(object):
    ...
    def __init__(self, region, vertices, vertex_boundary_markers, triangles, topology):
        ...
        self.triangulation_edges, self.edge_boundary_markers = self.make_triangulation_edges()
    ...
    def make_unique_triangulation_edges(self):
        ...
        # TODO: TEST THIS
        edges_unique = np.copy(self.triangulation_edges)
        num_edges = len(edges_unique)
        edges_unique = np.sort(edges_unique, axis=1)
        edges_unique = np.sort(edges_unique, axis=0)
        keep_indicator = np.full((num_edges,), True)
        for i in range(num_edges - 1):
            if np.all(edges_unique[i, :] == edges_unique[i + 1, :]):
                keep_indicator[i] = False
        edges_unique = edges_unique[keep_indicator, :]
        return edges_unique
